...
class FarmController extends BackendController
{
    ...
    public function actionCreate()
    {
        ...
        if ($isLoading && $model->validate()) {
            $model->save();
            \Yii::$app->session->setFlash('success', Yii::t('app/farm', 'FARM_CREATE_SUCCESS'));
            return $this->redirect(["farm/index"]);
        } else {
            \Yii::$app->session->setFlash('error', Yii::t('app/farm', 'FARM_CREATE_ERROR'));
            return $this->render('new',
                compact("model")
            );
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($isLoading && $model->validate()) {
            $model->save();
            \Yii::$app->session->setFlash('success', Yii::t('app/farm', 'FARM_EDIT_SUCCESS'));
            return $this->redirect(["farm/index"]);
        } else {
            \Yii::$app->session->setFlash('error', Yii::t('app/farm', 'FARM_EDIT_ERROR'));
            return $this->render('edit',
                compact('model')
            );
        }
    ...
    public function actionDelete($id)
    {
        ...
        $model = Farm::findOne($id);
        $model->delete();
        \Yii::$app->session->setFlash('success', Yii::t('app/farm', 'FARM_DELETE_SUCCESS'));
